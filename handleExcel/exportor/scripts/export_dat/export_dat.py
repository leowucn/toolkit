...
class Generator:
		...
		for i in range(0, colSize):
			if self.isSplitCase(lstName[i].lower()):
				...
				if len(typeList) != len(variablesList):
					raise  Exception('encounter some error here. mark 4.')
					return
				if len(typeList) > 0:
	...
	def ConverCell(self, typename, data, writeValue):
		if typename == "int" or typename == "byte":
			try:
				return int(data)
			except:
				print 'data type error!'
				raise
		elif typename == "string" or typename == "intarray" or typename == "intarray2":
			...
			if shortNum > 0:
				for i in range(shortNum):
					if typeList[len(contentList) -1 + i].lower() == "string":				
						contentList.append('')
					else:
...
def main():
	generator = Generator()
	generator.GenDatFile(unicode("角色总表.xls", 'gbk'))
	#generator.pause()
